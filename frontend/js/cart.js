...
async function showCart() {
    ...
    for (let i in cart) {
        await fetch(`https://julienjamet-kanap.herokuapp.com/api/products/${cart[i].id}`)
...
firstName.addEventListener("input", function (e) {
    ...
    if (/^([A-Z])([a-zéèç]+)(-[A-Z][a-zéèêïç]+)?$/.test(e.target.value)) {
...
address.addEventListener("input", function (e) {
    ...
    if (/^[1-9][0-9]{0,3}(?:[\s-][a-zéèêïçA-Z\-]+)*$/.test(e.target.value)) {
...
city.addEventListener("input", function (e) {
    ...
    if (/^[A-Z][a-zéèç]*(?:[\s-][a-zA-Z]+)*$/.test(e.target.value)) {
...
order.addEventListener("click", function (e) {
    ...
    /*Sinon, on constitue notre objet user avec les données saisies...*/
        ...
        function send() {
            fetch(`https://julienjamet-kanap.herokuapp.com/api/products/order`, {
                method: "POST",
                headers: {
